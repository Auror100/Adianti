...
class Mascara extends TPage
{
    public function __construct()
    {
        ...
        $element1->setMask('99.999-999'); // Apenas números e o banco vai adionar as marcações também        
        $element2->setMask('99.999-999',true); // Apenas números e o banco vai adicionar apenas eles
        $element3->setMask('99.999.999/9999-99');
        $element4->setMask('99.999.999/9999-99',true);
        $element5->setMask('A!'); // Letras e números infinitos
        $element6->setMask('AAA'); // 3 Letra e ou números
        $element7->setMask('S!'); // Letras infinitas
        $element8->setMask('SSS'); // 3 Letras
        $element9->setMask('9!'); // Números infinitos
        $element10->setMask('999'); // 3 números
        $element11->setMask('SSS-9A99');
        $element12->forceUpperCase();
        $element13->forceLowerCase();
        $element14->setNumericMask(2,',','.',true); // Marcação real centavo apenas no front
