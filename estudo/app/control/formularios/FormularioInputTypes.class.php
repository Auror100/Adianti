...
class FormularioInputTypes extends TPage
    public function __construct()
    {
        ...
        $radio = new TRadioGroup('radio'); // Radio
        $radio2 = new TRadioGroup('radio2');
        $check = new TCheckGroup('check'); // Check
        $check2 = new TCheckGroup('check2');
        $combo2 = new TCombo('combo');
        $combo = new TCombo('combo2');
        $select = new TSelect('select'); // Diferente do combo da para selecionar mais de uma opção no select
        $search =  new TMultiSearch('search'); // Tem uma caixa de pesquisa
        $unique =  new TUniqueSearch('unique'); // Da para acrescentar Tags no estilo SoundCloud
        $multi = new TMultiEntry('multi');
        $autocomp = new TEntry('autocomplete'); // Da para autocompletar o que foi feito
        ...
        $this->form->addFields([new TLabel('Multi Search')],[$unique]);
        $this->form->addFields([new TLabel('Unique Search')],[$multi]);
